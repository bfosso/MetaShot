...
for division in division_list:
    ...
    if os.path.exists(os.path.join(division, "%s_refined_classification_data.txt" % division)):
        ...
        cmd = shlex.split("ktImportTaxonomy -o %s -k %s -tax %s" % (
            division + "_krona.html", os.path.join(krona_folder, division + "_krona_data.tsv"),
            os.path.join(reference_path, "krona_tax")))
...
total.close()
cmd = shlex.split("ktImportTaxonomy -o %s -k %s -tax %s" % (
    "full_assignment_krona.html", os.path.join(krona_folder, "full_data_krona_data.tsv"),
    os.path.join(reference_path, "krona_tax")))
p = subprocess.Popen(cmd)
p.wait()
