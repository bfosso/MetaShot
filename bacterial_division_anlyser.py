...
for split in split2index.keys():
    ...
    cmd = shlex.split("python %s -1 %s -2 %s -d %s -s %s -f %s -t %s" % (
        os.path.join(script_path, "bacterial_mapper.py"), R1_string, R2_string, database, split, mapping_folder,
        threads))
            ...
            if split not in completed:
                ...
                if result == 0:
                    ...
                    if process_iteration < 5:
                        ...
                        database = split2index[split]
                        cmd = shlex.split(
                            "python %s -1 %s -2 %s -d %s -s %s -f %s -t %s" % (
                                os.path.join(script_path, "bacterial_mapper.py"), R1_string, R2_string, database, split,
                                mapping_folder, threads))
                        p = psutil.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
with open(os.path.join(bacterial_folder, "mapped_on_bacteria_total.txt"), "w") as match_file:
